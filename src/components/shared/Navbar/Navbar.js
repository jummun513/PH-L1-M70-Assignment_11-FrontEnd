...
const Navbar = () => {
    const { displayUser, setOpenModal, setDisplayUser } = useContext(MyContext);
                        ...
                        <ul className="flex flex-col md:text-white rounded md:rounded-none mt-3 md:mt-0 text-center md:flex-row md:space-x-6 bg-white md:bg-transparent lg:space-x-12">
                            {navigation.map((item) => (
                                <li id='sidebar' key={item.name}>
                                    <NavLink to={item.href} className=" text-sm font-semibold rounded flex justify-center py-3 md:px-3 md:py-2 hover:bg-primary hover:text-white md:hover:bg-primary ease-linear duration-150">
                                        {item.name}
                                    </NavLink>
                                </li>
                            ))}
